...
class BeautyAnalyzerPro:
    ...
    def __init__(self, model_path=None):
        ...
        default_model = os.path.join(script_dir, "models", "face_landmarker.task")
    ...
    def _create_landmarkers(self):
        if not os.path.isfile(self.model_path):
            raise FileNotFoundError(
                ...
                "Choose a .task model file or place face_landmarker.task under models/."
            )

        base_options = BaseOptions(model_asset_path=self.model_path)

        self.static_options = vision.FaceLandmarkerOptions(
        ...
        self.video_options = vision.FaceLandmarkerOptions(
            base_options=base_options,
            running_mode=vision.RunningMode.IMAGE,
            num_faces=1,
            min_face_detection_confidence=0.5,
        ...
        self.static_mesh = vision.FaceLandmarker.create_from_options(self.static_options)
        self.video_mesh = vision.FaceLandmarker.create_from_options(self.video_options)
    ...
    def _build_masks(self, frame_shape, lm):
        ...
        forehead_rect = self._rect_mask(
            (h, w),
            x + 0.22 * bw, y + 0.06 * bh,
            x + 0.78 * bw, brow_y - 0.03 * bh
        )
        left_cheek_rect = self._rect_mask(
            (h, w),
            x + 0.08 * bw, y + 0.35 * bh,
            x + 0.42 * bw, y + 0.72 * bh
        )
        right_cheek_rect = self._rect_mask(
            (h, w),
            x + 0.58 * bw, y + 0.35 * bh,
            x + 0.92 * bw, y + 0.72 * bh
        )
        nose_rect = self._rect_mask(
            (h, w),
            x + 0.42 * bw, y + 0.24 * bh,
            x + 0.58 * bw, y + 0.62 * bh
        )
    ...
    def analyze(self, frame_bgr, static_image=False):
        ...
        if not result.face_landmarks:
            cv2.putText(out, "No face detected", (18, 36), cv2.FONT_HERSHEY_SIMPLEX, 0.95, (0, 0, 255), 2, cv2.LINE_AA)
            response["frame"] = out
            return response

        h, w = frame_bgr.shape[:2]
        lm = self._tasks_landmarks_to_pixels(result.face_landmarks[0], w, h)
        ...
        base_heuristic = 100.0 * (
            0.72 * texture_score + 0.28 * geo["geo_score"]
        )
        ...
        if static_image:
            heuristic_score = base_heuristic
        else:
        ...
        face_poly = np.array([lm[i] for i in self.FACE_OVAL], dtype=np.int32)
        score_color = (0, 200, 0) if heuristic_score < 35 else ((0, 185, 255) if heuristic_score < 60 else (0, 0, 255))

        blend_mask(out, masks["forehead"], (255, 128, 0), 0.24)
        blend_mask(out, masks["left_cheek"], (255, 0, 180), 0.24)
        blend_mask(out, masks["right_cheek"], (180, 0, 255), 0.24)
        blend_mask(out, masks["nose"], (0, 180, 255), 0.24)

        cv2.polylines(out, [face_poly], True, score_color, 2, cv2.LINE_AA)
        cv2.circle(out, geo["face_center"], 4, (255, 255, 255), -1)

        line1 = f"Heuristic: {heuristic_score:.1f} [{heuristic_label}]"
        line2 = f"Model: {model_label} ({model_prob:.2f})"
        line3 = f"Geo big-eye/slim: {geo['big_eye_score']:.2f}/{geo['slim_face_score']:.2f}  frontal: {geo['frontality']:.2f}"
        line4 = f"Temp flicker/warp/jump: {temporal['flicker']:.2f}/{temporal['warp']:.2f}/{temporal['jump']:.2f}"

        cv2.putText(out, line1, (16, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.73, score_color, 2, cv2.LINE_AA)
        cv2.putText(out, line2, (16, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(out, line3, (16, 86), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (240, 240, 240), 2, cv2.LINE_AA)
        cv2.putText(out, line4, (16, 114), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (220, 220, 220), 2, cv2.LINE_AA)

        summary = (
            f"Heuristic={heuristic_score:.2f} ({heuristic_label})\n"
            f"Model={model_label}  prob={model_prob:.4f}\n"
            f"Texture score={texture_score:.4f}  Global skin={global_metrics['score']:.4f}\n"
            f"ROI forehead={forehead_metrics['score']:.4f}  left cheek={left_cheek_metrics['score']:.4f}  "
            f"right cheek={right_cheek_metrics['score']:.4f}  nose={nose_metrics['score']:.4f}\n"
            f"Geometry eye_ratio={geo['eye_ratio']:.4f}  eye_height_ratio={geo['eye_height_ratio']:.4f}  "
            f"jaw_cheek_ratio={geo['jaw_cheek_ratio']:.4f}  face_ar={geo['face_aspect_ratio']:.4f}  frontality={geo['frontality']:.4f}\n"
            f"Big-eye={geo['big_eye_score']:.4f}  Slim-face={geo['slim_face_score']:.4f}  Geo score={geo['geo_score']:.4f}\n"
            f"Temporal flicker={temporal['flicker']:.4f}  warp={temporal['warp']:.4f}  jump={temporal['jump']:.4f}  total={temporal['score']:.4f}\n"
            f"Global raw: lap_var={global_metrics['lap_var']:.4f}  residual={global_metrics['residual_mean']:.4f}  "
            f"chroma_std={global_metrics['chroma_std']:.4f}  edge_density={global_metrics['edge_density']:.4f}  "
            f"texture_ratio={global_metrics['texture_ratio']:.4f}"
        )
...
class VideoWorker(QThread):
    ...
    def run(self):
        ...
        while self._running:
            ...
            self.scores.append(result["heuristic_score"])
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.analyzer = BeautyAnalyzerPro()
        self.worker = None
        self.last_frame = None
        self.last_result = None

        self.setWindowTitle(f"Beauty Detector Pro - {os.path.basename(self.analyzer.model_path)}")
    ...
    def _build_ui(self):
        ...
        self.preview.setMinimumSize(920, 720)
        ...
        self.btn_train = QPushButton("Train Model (Folder)")
        ...
        for b in [self.btn_train, self.btn_load_model, self.btn_save_model, self.btn_reset_temporal, self.btn_choose_model]:
        ...
        g1l.addWidget(QLabel("Heuristic:"), 0, 0)
        g1l.addWidget(self.lbl_heuristic, 0, 1)
        g1l.addWidget(QLabel("Model:"), 1, 0)
        g1l.addWidget(self.lbl_model, 1, 1)
        g1l.addWidget(QLabel("Texture:"), 2, 0)
        g1l.addWidget(self.lbl_texture, 2, 1)
        g1l.addWidget(QLabel("Geometry:"), 3, 0)
        g1l.addWidget(self.lbl_geo, 3, 1)
        g1l.addWidget(QLabel("Temporal:"), 4, 0)
        g1l.addWidget(self.lbl_temporal, 4, 1)
        g1l.addWidget(QLabel("ROI:"), 5, 0)
        g1l.addWidget(self.lbl_roi, 5, 1)
        g1l.addWidget(QLabel("Rolling Avg:"), 6, 0)
        g1l.addWidget(self.lbl_roll, 6, 1)
        g1l.addWidget(QLabel("FPS:"), 7, 0)
        g1l.addWidget(self.lbl_fps, 7, 1)
        g1l.addWidget(self.bar, 8, 0, 1, 2)
    ...
    def choose_model(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self,
            "Choose MediaPipe Face Landmarker Model",
            "",
            "Task models (*.task)"
        )
        if not path:
        ...
        try:
            self.analyzer.set_model_path(path)
            self.setWindowTitle(f"Beauty Detector Pro - {os.path.basename(self.analyzer.model_path)}")
            self.log_msg(f"Model switched to: {path}")
            QMessageBox.information(self, "Model Loaded", f"Using model:\n{path}")
    ...
    def save_model(self):
        if self.analyzer.model is None:
            QMessageBox.information(self, "Info", "No trained/loaded model available.")
            return

        path, _ = QFileDialog.getSaveFileName(
            self, "Save Model", "beauty_logreg_model.npz", "NPZ (*.npz)"
        )
        if not path:
            return
        self.analyzer.model.save(path)
        self.log_msg(f"Model saved: {path}")

    def load_model(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Load Model", "", "NPZ (*.npz)"
        )
        if not path:
            return
        ...
        try:
            ...
            self.log_msg(f"Model loaded: {path}")
        except Exception as e:
